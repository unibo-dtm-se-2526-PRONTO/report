...
The first sequence diagram shows the defining end-to-end flow. Having chosen an office, the student submits a question, which the backend forwards to the FaqMatchingService; the service searches the office's FAQs using full-text and vector similarity search and returns the best match, if a sufficiently relevant one exists. If the office the student picked has no good match, the search does not simply come back empty: the service falls back to searching across every office's FAQs, and if a better match turns up elsewhere, it returns that answer together with the office that actually owns it. This turns a dead end into routing: instead of "no answer found", the student is told which office actually handles their question and can book there directly. The suggestion is then presented to the student, and the decision to stop there is theirs alone: the system never resolves a request on their behalf. If the student marks the answer as satisfactory, the flow ends and no appointment is created. If they do not (or no match was found at all), the client requests the relevant office's availability and the student picks a slot, which the BookingService turns into an appointment carrying the original question.
